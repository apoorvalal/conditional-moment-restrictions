...
def get_result_for_best_divergence(
    method, n_train, test_metric, validation_metric, func=None, optimizer=None
):
    if func is not None:
        opt = f"_{func}"
        ...
        opt = ""
        experiment = (
            "results/HeteroskedasticNoiseExperiment/HeteroskedasticNoiseExperiment"
        )
    ...
    for divergence in ["chi2", "log", "kl"]:
        ...
        try:
            ...
            print(
                'No such file or directory: "results/NetworkIVExperiment/NetworkIVExperiment_method=KernelFGEL-kl-lbfgs_n=2000_abs.json"'
            )
    ...
    for n_train in n_samples:
        for method in methods:
            if method in ["NeuralFGEL", "KernelFGEL"]:
                mses, mmrs = get_result_for_best_divergence(
                    method,
                    n_train,
                    test_metric="mse",
...
if __name__ == "__main__":
    ...
    plot_results_over_sample_size(
        [
            ...
            "KernelFGEL",
            "NeuralFGEL",
        ],  # 'OrdinaryLeastSquares', 'KernelMMR', 'KernelVMM', 'KernelFGEL'],
