...
class WMM(GeneralizedEL):
    ...
    def mmr_objective(self, x1, x2):
        ...
        objective = torch.einsum("ir, ij, jr -> ", psi1, self.kernel_z, psi2) / (
            psi1.shape[0] ** 2
        )
        return objective
    ...
    def _init_dual_params(self):
        self.dual_moment_func = Parameter(
            shape=(1, self.dim_psi)
        )  # Not used but required for parent class
        if self.move_variable == "y":
            self.y = torch.nn.Parameter(
                copy.deepcopy(self.x0[1].detach()), requires_grad=True
            )
            self.x = [self.x0[0], self.y]
            self.all_dual_params = [self.y]
        elif self.move_variable == "t":
            self.t = torch.nn.Parameter(
                copy.deepcopy(self.x0[0].detach()), requires_grad=True
            )
    ...
    def _objective(self, x, z, *args, **kwargs):
        if kwargs["which_obj"] == "theta":
            ...
            theta_obj = None
            particle_obj = (
                self.mmr_objective(self.x, self.x)
                + 1
                / (2 * self.tau)
                * torch.norm(torch.cat(self.x) - torch.cat(self.x0)) ** 2
            )
        else:
            theta_obj = self.mmr_objective(self.x, self.x)
            particle_obj = (
...
def eval_moment_func(data, model):
    psi = torch.norm(
        torch.mean(np_to_tensor(data["y"]) - model(np_to_tensor(data["t"])), dim=0),
        dim=-1,
    )
...
if __name__ == "__main__":
        ...
        for tau in taus:
            ...
            for _ in range(n_run):
                ...
                mses.append(
                    np.sum(
                        np.square(
                            np.squeeze(trained_model.get_parameters()) - exp.theta0
                        )
                    )
                )
                ...
                E_x_wmm_psi_theta0.append(
                    eval_moment_func(
                        data=estimator.get_shifted_data(), model=exp.eval_true_model
                    )
                )
                E_x0_psi_theta0.append(
                    eval_moment_func(data=exp.train_data, model=exp.eval_true_model)
                )

                E_x_wmm_psi_theta_wmm.append(
                    eval_moment_func(
                        data=estimator.get_shifted_data(), model=estimator.model
                    )
                )
                E_x0_psi_theta_wmm.append(
                    eval_moment_func(data=exp.train_data, model=estimator.model)
                )
                E_x_true_psi_theta_wmm.append(
                    eval_moment_func(data=exp.test_data, model=estimator.model)
                )
                E_x0_psi_theta_mmr.append(
                    eval_moment_func(data=exp.train_data, model=trained_model)
                )
    ...
    ax[0].fill_between(
        taus,
        np.subtract(
            res["wmm_mean"], np.asarray(res["wmm_std"]) / np.sqrt(res["n_run"])
        ),
    ...
    ax[0].fill_between(
        taus,
        np.subtract(
            res["mmr_mean"], np.asarray(res["mmr_std"]) / np.sqrt(res["n_run"])
        ),
    ...
    ax[1].plot(
        taus, res["list_E_x0_psi_theta0"], label=r"$\hat{P}_{n}$", color="purple"
    )
    ...
    ax[2].plot(
        taus,
        res["list_E_x_wmm_psi_theta_wmm"],
        label=r"$P_{WMM}$, $\theta_{WMM}$",
        color="orange",
    )
    ax[2].plot(
        taus,
        res["list_E_x0_psi_theta_wmm"],
        label=r"$\hat{P}_{n}$, $\theta_{WMM}$",
        color="purple",
    )
    ax[2].plot(
        taus,
        res["list_E_x_true_psi_theta_wmm"],
        label=r"$P_{0}$, $\theta_{WMM}$",
        color="green",
    )
    ax[2].plot(
        taus,
        res["list_E_x0_psi_theta_mmr"],
        label=r"$\hat{P}_{n}$, $\theta_{MMR}$",
        color="blue",
    )
