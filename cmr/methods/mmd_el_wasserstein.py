...
def objective_MR(rkhs_params, moment_func, x1, y1, x2, y2, dual_vec, reg_param=100):
    ...
    obj = torch.einsum("ai,bi->b", dual_vec, moment_func([x2, y2])) + prox_term / (
        2 * 1e-8
    )
...
def objective_CMR(
    rkhs_params, moment_func, x1, y1, z, x2, y2, dual_func, reg_param=100, z_dep=False
):
    """
    Compute objective for the Wasserstein GEL method for unconditional Moment Restrictions.
...
class KMMWasserstein(KMM):
    ...
    def _init_dual_params(self):
        ...
        # Initialize particle
        self.particles = ParameterVector(
            dim_x=self.x_samples[0].shape[0],
            dim_y=self.x_samples[0].shape[1] + self.x_samples[0].shape[1],
            init_value=torch.hstack(self.x_samples),
        )  # TODO: fix this initialization
        # self.p_opt = optim.Adam(params=self.particles.parameters(), lr=1e-2,
        #                         betas=(0.5, 0.9))
        self.p_opt = optim.LBFGS(
            params=self.particles.parameters(),
    ...
    def _objective(self, x, z, *args, **kwargs):
        if self.n_rff == 0:
            ...
            rkhs_norm_sq = torch.einsum(
                "i, i ->", self.rkhs_func.params[:, 0], self.rkhs_func.params[:, 0]
            )
        else:
        ...
        particle_optimizaton(
            rkhs_params=self.rkhs_func.params,
            x=x,
            z=z,
        ...
        objective = (
            torch.mean(rkhs_func)
            + self.dual_normalization.params
            - 1 / 2 * rkhs_norm_sq
        )

        if self.l2_lambda > 0:
            regularizer = self.l2_lambda * torch.mean(
                self._eval_dual_moment_func(self.z_samples) ** 2
            )
        else:
            regularizer = 0
...
if __name__ == "__main__":
    ...
    test_cmr_estimator(
        estimation_method="MMDEL-neural", n_runs=1, n_train=30, hyperparams=None
    )
    test_cmr_estimator(
        estimation_method="RF-MMDEL", n_runs=1, n_train=30, hyperparams=None
    )
