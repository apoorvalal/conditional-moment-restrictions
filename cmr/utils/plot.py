...
def load_data_to_df(filepath):
    ...
    df = pd.concat([df, df["hyperparam"].apply(pd.Series)], axis=1).drop(
        columns="hyperparam"
    )
...
def get_mean_and_sem(
    df, test_metric="test_risk", val_metric="val_loss", hparam_config=None
):
...
def load_and_merge_datasets(filepaths, property_dict=None, merge="hparam_configs"):
    ...
    for filepath, prop in zip(filepaths, vals):
        ...
        data_frame = data_frame.drop(
            columns=["test_risk_optim", "parameter_mse_optim", "best_index"]
        )
...
def plot_results_over_sample_size(
    methods, n_samples, labels=None, hparam_config=None, logscale=False
):
    ...
    for n_train in n_samples:
        for method in methods:
            ...
            filename = [
                base_path
                / f"results/heteroskedastic_one/heteroskedastic_one_method={m}_n={n_train}.json"
                for m in method
            ]
            df = load_and_merge_datasets(filename)
            mean, sem = get_mean_and_sem(
                df,
                test_metric="mse",
                val_metric="val_loss",
...
def plot_divergence_comparison(
    n_samples, validation_metric, logscale=False, remove_failed=False, optimizer="lbfgs"
):
    plt.rcParams.update(NEURIPS_RCPARAMS)
    sns.set_theme()
    marker = ["v", "o", "s", "d", "p", "*", "h"]
...
def generate_table_network_iv(n_train, methods, hparam_config=None):
    ...
    for func in funcs:
        for method in methods:
            ...
            filename = [
                base_path
                / f"results/network_iv/network_iv_method={m}_n={n_train}_{func}.json"
                for m in method
            ]
            df = load_and_merge_datasets(filename)
            mean, sem = get_mean_and_sem(
        ...
        table.append(
            ...
            + [
                r"${:.2f}\pm{:.2f}$".format(
                    ...
                    results[func][model]["sem"] * 1e1,
                )
                for model in results[func].keys()
            ]
        )
...
def generate_table_bennett_hetero(n_trains, methods, hparam_config):
    ...
    results = {
        n_train: {"".join(model): {} for model in methods} for n_train in n_trains
    }
    for n_train in n_trains:
        for method in methods:
            ...
            filename = [
                base_path
                / f"results/bennet_hetero/bennet_hetero_method={m}_n={n_train}.json"
                for m in method
            ]
            df = load_and_merge_datasets(filename)
            mean, sem = get_mean_and_sem(
    ...
    for n_train in n_trains:
        table.append(
            ...
            + [
                r"${:.2f}\pm{:.2f}$".format(
                    results[n_train][model]["mean"], results[n_train][model]["sem"]
                )
                for model in results[n_train].keys()
            ]
        )
    print(tabulate(table, tablefmt="latex_raw"))
...
if __name__ == "__main__":
    ...
    generate_table_bennett_hetero(
        n_trains=[2000, 4000], methods=methods, hparam_config=None
    )  # {'entropy_reg_param': 1})
