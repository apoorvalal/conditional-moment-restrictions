...
def estimation(
    ...
    if estimation_method not in (mr_estimators + cmr_estimators):
        raise NotImplementedError(
            f"Invalid estimation method specified, pick `estimation_method` from "
            f"{set(mr_estimators + cmr_estimators)}."
        )

    if conditional_mr and estimation_method in mr_estimators:
        print(
            "Solving conditional MR problem with method for unconditional MR, "
            "ignoring instrument data `train_data['z']`."
        )

    if not conditional_mr and estimation_method in cmr_estimators:
        raise RuntimeError(
            "Specified method requires conditional MR but the provided problem is an unconditional MR. "
            f"Provide instrument data `train_data['z']` or choose `estimation_method` for "
            f"unconditional MR from {mr_estimators}."
        )

    if hyperparams is not None:
        assert np.alltrue([isinstance(h, list) for h in list(hyperparams.values())]), (
            "`hyperparams` arguments must be of the form {key: list}"
        )
    ...
    if normalize_moment_function:
        model, moment_function = pretrain_model_and_renormalize_moment_function(
            moment_function, model, train_data, conditional_mr
        )

    # Train estimator for different hyperparams and return best model (models for other hparams also stored)
    trained_model, train_statistics = optimize_hyperparams(
        model=model,
...
def pretrain_model_and_renormalize_moment_function(
    moment_function, model, train_data, conditional_mr
):
    ...
    if conditional_mr and train_data["z"].shape[0] < 5000:
        ...
        estimator = OrdinaryLeastSquares(
            model=copy.deepcopy(model), moment_function=moment_function
        )
    ...
    normalization = torch.Tensor(
        np.std(
            moment_function(
                pretrained_model(torch.Tensor(train_data["t"])),
