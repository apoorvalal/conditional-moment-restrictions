...
class KMMAnalysis(KMM):
    ...
    def eval_rkhs_func(self):
        return (
            (
                torch.einsum("ij, ik -> k", self.rkhs_func.params, self.kernel_x)
    ...
    def _init_dual_params(self):
        ...
        self.all_dual_params = list(self.dual_normalization.parameters()) + list(
            self.rkhs_func.parameters()
        )
    ...
    def _objective(self, x, z, *args, **kwargs):
        ...
        if self.n_rff > 0:
            rkhs_norm_sq = torch.einsum(
                "ir, ij -> j", self.rkhs_func.params, self.rkhs_func.params
            )
        else:
            # rkhs_norm_sq = torch.einsum('ir, ij, jr ->', self.rkhs_func.params, self.kernel_x, self.rkhs_func.params)
            rkhs_norm_sq = (
                torch.norm(self.rkhs_func.params.t() @ self.kernel_x_cholesky) ** 2
            )

        exponent = (
            rkhs_func
            + self.dual_normalization.params
            - self.eval_psi_h(x, kwargs["numpy"])
        )
        objective = (
            expected_rkhs_func
            + self.dual_normalization.params
            - 1 / 2 * rkhs_norm_sq
            - self.kl_reg_param
            * torch.mean(self.f_divergence(1 / self.kl_reg_param * exponent))
        )
    ...
    def _optimize_dual_params_gd(self, x_tensor, z_tensor, iters):
        self.dual_func_optimizer = torch.optim.Adam(
            params=self.all_dual_params, lr=1e-3
        )
        rkhs_fun = []
        loss = []
    ...
    def _optimize_dual_params_cvxpy(self, x_tensor, z_tensor, f_divergence):
        with torch.no_grad():
            ...
            exponent = cvx.sum(
                kernel_x.T @ rkhs_func + dual_normalization - dual_func_psi, axis=1
            )
            if f_divergence == "exact":
                ...
                objective += (
                    -self.kl_reg_param
                    / n_sample
                    * cvx.sum(div(1 / self.kl_reg_param * exponent))
                )
                constraints = []
