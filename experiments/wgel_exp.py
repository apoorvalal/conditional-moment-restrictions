...
from experiments.mmr_exp import (
    ParameterVector,
    HeteroskedasticNoiseExperiment,
    LinearModel,
)
...
def objective_MR(
    moment_func, model, x1, y1, x2, y2, dual_vec, sup=False, reg_param=100
):
    """
...
def objective_CMR(
    moment_func, model, x1, y1, z, x2, y2, dual_func, sup=False, reg_param=100
):
    """
    Compute objective for the Wasserstein GEL method for unconditional Moment Restrictions.
...
def particle_optimizaton(
    ...
    for i in range(n_iter):
        ...
        if np.isclose(np.mean(loss_window), delta_mean, atol=1e-5, rtol=0.0):
            ...
            loss_window.popleft()
    print(
        "Particle update L2 norm: {0} achieved in {1} steps".format(loss_window[-1], i)
    )
...
def run_wgel(args):
    ...
    if args.exp == "hetero":
        ...
        if args.pretrain:
            model = ParameterVector(
                exp.dim_theta, 1, init_value=trained_model.get_parameters()[0]
            )
        else:
            model = ParameterVector(exp.dim_theta, 1)

        print(
            f"True param: {exp.poisson_param} \n"
    ...
    #                     max_iter=100)
    p_opt = optim.Adam(
        params=particles.parameters(), lr=args.particle_lr, betas=(0.5, 0.9)
    )
    m_opt = optim.Adam(params=model.parameters(), lr=args.theta_lr, betas=(0.5, 0.9))
    # dual_opt = optim.LBFGS(params=dual_vec.parameters(),
    #                        line_search_fn='strong_wolfe',
    #                        max_iter=100)
    dual_opt = optim.Adam(
        params=dual_func.parameters(), lr=args.dual_lr, betas=(0.5, 0.9)
    )
    ...
    for i in range(args.wgel_iter):
        ...
        print(
            "Loss at {0}-th iteration: {1} and Model parameters: {2}".format(
                i, loss_theta[-1], model.get_parameters()
            )
        )
...
parser.add_argument(
    "--conditional", default=True, action=argparse.BooleanOptionalAction
)
