...
def run_experiment(
    ...
    trained_model, full_results = estimation(
        model=exp.get_model(),
    ...
    for model in full_results["models"]:
        ...
        if exp.get_true_parameters() is not None:
            parameter_mses.append(
                float(
                    np.mean(
                        np.square(
                            np.squeeze(model.get_parameters())
                            - np.squeeze(exp.get_true_parameters())
                        )
                    )
                )
            )
    ...
    result = {
        "test_risk_optim": test_risks[full_results["best_index"]],
...
def run_parallel(
    ...
    estimator_method_list = [
        copy.deepcopy(estimation_method) for _ in range(repititions)
    ]
    estimator_kwargs_list = [
        copy.deepcopy(estimator_kwargs) for _ in range(repititions)
    ]
...
def run_experiment_repeated(
    ...
    file = (
        f"results/{exp_name}/{run_dir}/{exp_name}_method={estimation_method}_n={n_train}"
        + str(filename)
        + ".json"
    )
    try:
...
if __name__ == "__main__":
    ...
    if args.exp_option is not None:
        exp_info["exp_params"] = {
            list(exp_info["exp_params"].keys())[0]: args.exp_option
        }
        filename = "_" + args.exp_option
    else:
        filename = ""

    results = run_experiment_repeated(
        experiment=exp_info["exp_class"],
