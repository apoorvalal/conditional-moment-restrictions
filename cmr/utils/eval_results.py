...
def get_result_for_best_divergence(
    method, n_train, test_metric, experiment=None, func="", run_dir=""
):
    ...
    test_metrics = np.asarray(
        [test_metrics[index][i] for i, index in enumerate(indices)]
    )
    return test_metrics, validation_metrics


def plot_mr_over_sample_size(
    methods,
    n_samples,
    kl_reg_params=None,
    logscale=False,
        ...
        for method in methods:
            if method == "GEL":
                ...
                for key, val in res.items():
                    results[f"GEL, divergence={key}"]["mean"].append(
                        val["mean_square_error"]
                    )
                    results[f"GEL, divergence={key}"]["std"].append(
                        val["std_square_error"]
                    )
            elif method == "KEL" and kl_reg_params is not None:
                res = separate_kel_by_reg_param(
                    reg_params=kl_reg_params, n_train=n_train, experiment="poisson"
                )
                for key, val in res.items():
                    results[f"KEL, kl_reg={key}"]["mean"].append(
                        val["mean_square_error"]
                    )
                ...
                results[method]["std"].append(
                    np.std(test_error) / np.sqrt(len(test_error))
                )
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(BASE_PATH / "/results/Poisson_separated.pdf", dpi=200)
    plt.show()


def get_test_metric_over_sample_size(
    methods,
    n_samples,
    experiment,
    test_metric="mse",
    ...
    for n_train in n_samples:
        for method in methods:
            if method in [
                ...
                test_error, val_error = get_result_for_best_divergence(
                    method,
                    n_train,
            ...
            elif method in ["KernelELKernel"] and kl_reg_param is not None:
                res = separate_kel_by_reg_param(
                    reg_params=[kl_reg_param],
                    n_train=n_train,
                    experiment=experiment,
                    method=method,
                )
                ...
                path = BASE_PATH / f"results/{experiment}/{run_dir}/{experiment}"
                filename = f"{path}_method={method}_n={n_train}.json"
                res = load_and_summarize_results(filename)
...
def plot_results_over_sample_size(
    methods,
    n_samples,
    experiment="poisson",
    test_metric="mse",
    ...
    results = get_test_metric_over_sample_size(
        methods=methods,
        n_samples=n_samples,
        experiment=experiment,
        test_metric=test_metric,
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(BASE_PATH / f"results/{experiment}_mse_over_n.pdf", dpi=200)
    plt.show()
...
def separate_gel_by_divergence(n_train=None):
    ...
    results = results_and_summary["results"]
    separate_results = {
        divergence: {"test_risk": [], "mse": [], "hyperparam": [], "val_loss": []}
        for divergence in divergences
    }
    best_separate_results = {
        divergence: {"test_risk": [], "mse": [], "hyperparam": [], "val_loss": []}
        for divergence in divergences
    }
    ...
    for res in results:
        for i, hyper in enumerate(res["hyperparam"]):
            ...
            separate_results[hyper["divergence"]]["test_risk"].append(
                res["test_risk"][i]
            )
            separate_results[hyper["divergence"]]["mse"].append(res["mse"][i])
            separate_results[hyper["divergence"]]["hyperparam"].append(
                res["hyperparam"][i]
            )
        ...
        for divergence in divergences:
            ...
            try:
                ...
                best_separate_results[divergence]["test_risk"].append(
                    separate_results[divergence]["test_risk"][best_index]
                )
                best_separate_results[divergence]["mse"].append(
                    separate_results[divergence]["mse"][best_index]
                )
    ...
    for divergence in divergences:
        ...
        final_result[divergence] = {
            "mean_square_error": np.mean(best_separate_results[divergence]["mse"]),
...
def separate_kel_by_reg_param(
    ...
    results = results_and_summary["results"]
    best_separate_results = {
        reg_param: {"test_risk": [], "mse": [], "hyperparam": [], "val_loss": []}
        for reg_param in reg_params
    }

    for res in results:
        separate_results = defaultdict(
            lambda: {"test_risk": [], "mse": [], "hyperparam": [], "val_loss": []}
        )

        for i, hyper in enumerate(res["hyperparam"]):
            # Separate results of a single run by kl_reg_param
            separate_results[hyper["kl_reg_param"]]["test_risk"].append(
                res["test_risk"][i]
            )
            separate_results[hyper["kl_reg_param"]]["mse"].append(res["mse"][i])
            separate_results[hyper["kl_reg_param"]]["hyperparam"].append(
                res["hyperparam"][i]
            )
            separate_results[hyper["kl_reg_param"]]["val_loss"].append(
                res["val_loss"][i]
            )
        for kl_reg in reg_params:
            ...
            try:
                ...
                best_separate_results[kl_reg]["test_risk"].append(
                    separate_results[kl_reg]["test_risk"][best_index]
                )
                best_separate_results[kl_reg]["mse"].append(
                    separate_results[kl_reg]["mse"][best_index]
                )
                # best_separate_results[kl_reg]['hyperparam'].append(separate_results[kl_reg]['hyperparam'][best_index])
                # best_separate_results[kl_reg]['val_loss'].append(separate_results[kl_reg]['val_loss'][best_index])
            except ValueError:
    ...
    for kl_reg in reg_params:
        # Summarize over the n_runs rollouts
        final_result[kl_reg] = {
            "mean_square_error": np.mean(best_separate_results[kl_reg]["mse"]),
...
def plot_divergence_comparison_cmr(
    n_samples,
    kl_reg_params=None,
    logscale=False,
    remove_failed=False,
    ...
    # else:
    figsize = (LINE_WIDTH * 0.7, LINE_WIDTH * 0.7)
    fig, ax = plt.subplots(1, 1, figsize=figsize)
    ax = [ax]
    ...
    for k, version in enumerate(["Neural"]):
        ...
        for n_train in n_samples:
            for method in methods:
                filename = (
                    BASE_PATH / "results/HeteroskedasticNoiseExperiment" / run_dir
                )
                filename = (
                    filename
                    / "HeteroskedasticNoiseExperiment_method={method}_n={n_train}.json"
                )
                res = load_and_summarize_results(filename)
                ...
                if (
                    method == "KernelELKernel"
                    or method == "KernelELNeural"
                    or method == "KernelELNeural-log"
                ) and kl_reg_params is not None:
                    res = separate_kel_by_reg_param(
                        reg_params=kl_reg_params,
                        n_train=n_train,
                        experiment="HeteroskedasticNoiseExperiment",
                        method=method,
                    )
                    for kl_reg, r in res.items():
                        results[method + f"_{kl_reg}"]["mean"].append(
                            r["mean_square_error"]
                        )
                        results[method + f"_{kl_reg}"]["std"].append(
                            r["std_square_error"] / np.sqrt(r["n_runs"])
                        )
                else:
...
def generate_table(
        ...
        for method in methods:
            try:
                if method in ["NeuralFGEL", "KernelFGEL", "KernelELNeural"]:
                    test, val = get_result_for_best_divergence(
                        method=method,
                        n_train=n_train,
                        test_metric=test_metric,
                        experiment=experiment,
    ...
    for func in funcs:
        table.append(
            ...
            + [
                r"${:.2f}\pm{:.2f}$".format(
                    results[func][model]["mean"] * 1e1,
                    results[func][model]["std"] * 1e1,
                )
...
def generate_table_bennet(
    n_trains, test_metric="test_risk", experiment="benett_simple", run_dir=""
):
    methods = [
        "OLS",
        # 'KernelMMR',
    ...
    for n_train in n_trains:
        table.append(
            ...
            + [
                r"${:.2f}\pm{:.2f}$".format(
                    results[n_train][model]["mean"], results[n_train][model]["std"]
                )
                for model in methods
            ]
        )
