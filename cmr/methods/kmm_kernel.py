...
class KMMKernel(KMM):
    def __init__(self, model, moment_function, verbose=0, **kwargs):
        ...
        if kwargs["n_rff_instrument_func"] is None:
            ...
            assert self.n_rff is not None, (
                "If RFF is used for instrument func, need to use it for RKHS function too."
            )
    ...
    def _init_dual_params(self):
        ...
        if self.n_rff_instrument_func:
            self.dual_moment_func = Parameter(
                shape=(self.n_rff_instrument_func, self.dim_psi)
            )
        else:
            self.dual_moment_func = Parameter(
                shape=(self.kernel_z.shape[0], self.dim_psi)
            )
    ...
    def _init_rff_instrument_func(self, z):
        ...
        self._eval_rff_instrument_func = rff.layers.GaussianEncoding(
            sigma=sigma_z,
    ...
    def _eval_dual_moment_func(self, z):
        if self.n_rff_instrument_func:
            return torch.einsum(
                ...
                self._eval_rff_instrument_func(z),
            )
        else:
            return torch.einsum(
                "ij, ki -> kj", self.dual_moment_func.params, self.kernel_z
            )

    def rkhs_norm_sq_instrument_func(self):
        if self.n_rff_instrument_func:
            return torch.einsum(
                "ij, ij ->", self.dual_moment_func.params, self.dual_moment_func.params
            )
        else:
            return torch.einsum(
                "ir, ij, jr ->",
