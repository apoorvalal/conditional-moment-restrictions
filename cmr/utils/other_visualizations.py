...
def plot_minorizing():
    ...
    ax.plot(
        x[0], y_exact, label=r"$f(x) + \eta$", color=colors[2], linestyle=linestyles[2]
    )
    ax.set_xlabel("x")
    ax.set_yticks([])
    ax.set_xticks([])
...
def plot_heteroskedastic():
    ...
    x = np.linspace(
        np.min(exp.train_data["t"]), np.max(exp.train_data["t"]), 1000
    ).reshape((-1, 1))
    ...
    ax.plot(
        x, eval_model(x, exp.theta0, numpy=True), color="red", label=r"$x^T \theta_0$"
    )
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_yticks([])
